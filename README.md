...
```
Script sẽ tự động:
1. Cài đặt các gói `node_modules` (nếu chưa có).
...
* Cơ chế kiểm soát tương tranh: Sử dụng khóa luồng tĩnh `BookingLock.LOCK` trong JVM kết hợp ghi đĩa nguyên tử `StandardCopyOption.ATOMIC_MOVE` để ngăn chặn hoàn toàn việc mất mát dữ liệu hoặc lỗi ghi đè file.
